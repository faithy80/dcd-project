...
def update_quantity_in_category(category):
    ...
    mongo.db.recipe_categories.update(
        {'name': category}, {'$set': {'number_of_recipes': counter}}
    )


def validate_form(form, collection):
    """ Returns an error list if the recipe, recipe collection or
    review form fails on validation
    """
...
def validate_image(image):
    """ Returns image URL if the URL is an image,
...
def search():
    """ Returns all recipes, recipes by category, all appliances
    or appliances by category and both categories
    """
    ...
    if request.args['collection'] == 'recipes':
        if request.args['find'] == 'all':
            ...
            recipes = mongo.db.recipes.find(
                {'title': {'$gte': start}}).sort('title').limit(limit)
        ...
        # searches for all recipes in the selected category
            ...
            page_header = (
                'Recipes in the "{}" category search results:'.format(
                    request.args['find'])
            )

            # determines all recipe titles in ascendent order
            get_titles = mongo.db.recipes.find(
                {'category': request.args['find']}).sort('title')
            ...
            # determines the subset
            recipes = mongo.db.recipes.find(
                {'category': request.args['find'], 'title': {'$gte': start}}
                ).sort('title').limit(limit)
    ...
    # searches for the recipe categories
        ...
        recipe_categories = mongo.db.recipe_categories.find(
            {'name': {'$gte': start}}).sort('name').limit(limit)
    ...
    # searches for all the appliances
        ...
        if request.args['find'] == 'all':
            ...
            # determines the subset
            appliances = mongo.db.appliances.find(
                {'brand': {'$gte': start}}).sort('brand').limit(limit)
        ...
        # searches for all appliances in the selected category
            ...
            page_header = (
                'Appliances in the "{}" category search results:'.format(
                    request.args['find'])
            )

            # determines all appliance brands in ascendent order
            get_brands = mongo.db.appliances.find(
                {'type': request.args['find']}).sort('brand')
            ...
            # determines the subset
            appliances = mongo.db.appliances.find(
                {'type': request.args['find'], 'brand': {'$gte': start}}
                ).sort('brand').limit(limit)
    ...
    # searches for the appliance categories
        ...
        if offset - limit < 0:
            ...
            prev_link = url_for(
                'search',
                collection='appliance_categories',
                limit=str(limit),
                offset=str(offset - limit)
            )
        ...
        # determines the subset
        appliance_categories = mongo.db.appliance_categories.find(
            {'name': {'$gte': start}}).sort('name').limit(limit)
...
def navsearch():
    ...
    get_titles = mongo.db.recipes.find(
        {'ingredients': {'$in': [search_text]}}).sort('title')
    ...
    # determines the subset
    recipes = mongo.db.recipes.find(
        {'ingredients': {'$in': [search_text]}, 'title': {'$gte': start}}
        ).sort('title').limit(limit)
...
def insert_recipe():
    """ Inserts a recipe into the database and redirects
...
def insert_recipe_category():
    """ Inserts a recipe category into the database and redirects
    to the list of all recipe categories
    """
...
def update_recipe(db_id):
    """ Updates a recipe in the database and redirects
    to the list of all recipes
    """
...
def update_recipe_category(db_id):
    """ Updates a recipe category in the database and redirects
    to the list of all recipe categories
    """
    ...
    if error_list == []:
        ...
        # keeps the old category name in case of change
        previous_name = mongo.db.recipe_categories.find_one(
            {'_id': ObjectId(db_id)})['name']
        ...
        # updates the category name to the new one in the correspondent recipes

        mongo.db.recipes.update_many({'category': previous_name}, {'$set': {
...
def delete_recipe(db_id):
    ...
    category = mongo.db.recipes.find_one(
        {'_id': ObjectId(db_id)})['category']
...
def delete_recipe_category(db_id):
    """ Removes a recipe category and all the recipes
    in the category from the database
    """

    # determines the category name by id
    category_name = mongo.db.recipe_categories.find_one(
        {'_id': ObjectId(db_id)})['name']
...
def add_review(db_id):
    """ Adds a review to a recipe or an appliance
    and refreshes the view page
    """
